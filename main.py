...
class Player:
    def __init__(self, x, y):
        ...
        self.leftRunningImages = [
            'leftrunningshaolin', '1.5leftrunningshaolin',
            '2leftrunningshaolin'
        ]
        self.rightRunningImages = [
            'rightrunningshaolin', '1.5rightrunningshaolin',
            '2rightrunningshaolin'
        ]
        ...
        self.deadplayerImages = [
            'dead1shaolin', 'dead2shaolin',
            'dead3shaolin', 'dead4shaolin'
        ]
    ...
    def update(self):
        ...
        if self.jumping:
            ...
            self.state = (
                "jump_left" if self.leftR else "jump_right"
            )
        ...
        if self.jumping:
            frames = (
                self.jumpleftImages if self.leftR
                else self.jumprightImages
            )

            if self.state in ("jump_left", "jump_right"):
                self.frameJUMP = 0
                self.frameCounter = 0
        ...
        if self.punch:
            frames = (
                self.punchleftImages if self.leftR
                else self.punchrightImages
            )
    ...
    def update(self):
        ...
        if self.frameCounter >= self.frameSpeed:
            ...
            self.frame = (
                ...
                len(self.robotLeftFrames)
                if self.left else len(self.robotRightFrames)
            )
    ...
    if estado_do_jogo == "jogo":
        ...
        if move.actor:
            screen.blit(
                move.actor.image,
                (
                    move.actor.x - world1.camera_x - move.actor.width / 2,
        ...
        screen.draw.filled_rect(
            Rect(
                ground.x - world1.camera_x, ground.y,
        ...
        for block in blocks:
            screen.blit(
                "block",
                (block.x - world1.camera_x, block.y)
            )
        ...
        for robot in robots:
            screen.blit(
                robot.actor.image,
                (
                    robot.actor.x - world1.camera_x - robot.actor.width / 2,
                    robot.actor.y - robot.actor.height / 2
                )
            )
...
def update():
    ...
    if move.playerDead and move.actor is None:
        estado_do_jogo = "menu"
        return
...
def colisao():
    ...
    for block in blocks:
        if move.actor.colliderect(block):
            ...
            if (
                move.falling
                and move.actor.bottom >= block.top
                and move.actor.y < block.top
            ):
            ...
            if (
                move.jumping
                and move.actor.top <= block.bottom
                and move.actor.y > block.bottom
            ):
    ...
    if move.punch:
        ...
        for robot in robots:
            robot_rect = Rect(
                robot.actor.x - robot.actor.width / 2,
                robot.actor.y - robot.actor.height / 2,
                robot.actor.width,
                robot.actor.height
            )
    ...
    if move.falling:
        ...
        for robot in robots:
            ...
            if move.actor.colliderect(robot_rect):
                ...
                if (
                    move.actor.bottom >= robot_rect.top
                    and move.actor.y < robot_rect.top
                ):
                    robot.dead = True
                    sounds.botdeath.play()
    ...
    for robot in robots:
        ...
        if move.actor.colliderect(robot_rect):
            ...
            if (
                not robot.dead
                and not move.punch
                and move.actor.right >= robot_rect.left
                and move.actor.left < robot_rect.left
            ):
                move.playerDead = True
                print('tocou')
                return
            ...
            if (
                not robot.dead
                and not move.punch
                and move.actor.left <= robot_rect.right
                and move.actor.right > robot_rect.right
            ):
                move.playerDead = True
                print('tocou tambem')
                return
